	...
	$.fn.swfupload = function(){
		...
		return this.each(function(){
			...
			if (args.length == 1 && typeof(args[0]) == 'object') {
				...
				swfu = $(this).data('__swfu');
				if (swfu && methodName == 'destroy') {
					$('.upload-button', this).each(function() {
						$('object', this).remove();
						$('.ajax', this).before('<span id="spanButtonPlaceholder-' + object.attr('id') + '"></span>');
					});

					swfu.destroy();
					$(this).data('__swfu', null)
				}	else if (swfu && swfu[methodName]) {
